#!/usr/bin/env python3
"""
ASL Detection Server with Vision Transformer Support
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, num_classes=29,
...
# -------------------- ASL MODEL CLASS --------------------
class ASLModel:
    def __init__(self, model_path="models/asl_vit_model.pth", model_type="vit"):
        self.model_path = model_path
        self.model_type = model_type
        self.model = None
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        
        # Set transform based on model type
        if model_type == "vit":
    ...
    def create_model(self, model_type, num_classes=29):
        ...
        if model_type == "vit":
            ...
            # Smaller ViT for faster inference
            return VisionTransformer(
                img_size=224,
                patch_size=16,
                num_classes=num_classes,
            ...
            # Tiny ViT for even faster inference (like your training code)
            return VisionTransformer(
                img_size=224,
                patch_size=16,
                num_classes=num_classes,
...
<head>
    ...
    <title>ASL Detection Studio - Vision Transformer</title>
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Real-time American Sign Language Recognition with Advanced AI</p>
            <div class="model-info">
                <span class="model-badge" id="modelBadge">Multi-Model AI System</span>
                <div style="margin-top: 10px; font-size: 0.9em; opacity: 0.8;">
                    Supporting Graph Neural Networks, Vision Transformers, and Convolutional Networks
                </div>
            </div>
        </div>
...
def health_check():
    ...
    return jsonify({
        ...
        'model_type': asl_model.model_type if asl_model else None,
        'timestamp': time.time()
    })
...
def initialize_model():
    ...
    model_configs = [
        ...
        ("models/asl_vit_model.pth", "vit"),
        ("asl_vit_model.pth", "vit"),
        ("models/asl_vit_small_model.pth", "vit_small"),
        ("models/asl_vit_tiny_model.pth", "vit_tiny"),
        
        # ResNet models (fallback)
        ("models/asl_resnet_model.pth", "resnet"),
        ("asl_resnet_model.pth", "resnet"),
        
        # Other existing models
        ("models/fast_asl_gnn_final.pth", "resnet"),  # Assume ResNet-like architecture
    ]
    
    for model_path, model_type in model_configs:
        try:
            asl_model = ASLModel(model_path, model_type)
            if asl_model.model is not None:
                print(f"✅ Successfully loaded {model_type.upper()} model from: {model_path}")
                return True
        except Exception as e:
            print(f"❌ Failed to load {model_type} model from {model_path}: {e}")
            continue
...
if __name__ == '__main__':
    print("🚀 Starting ASL Detection Server with Vision Transformer Support...")
    print("=" * 60)
    ...
    if not model_loaded:
        ...
        print("    - models/asl_vit_model.pth (full ViT)")
        print("    - models/asl_vit_small_model.pth (small ViT)")
        print("    - models/asl_vit_tiny_model.pth (tiny ViT)")
        print("  ResNet models:")
        print("    - models/asl_resnet_model.pth")
        print("    - models/fast_asl_gnn_final.pth")
        print("\nIf your model has a different architecture, you may need to modify the create_model() method.")
    else:
        print(f"\n✅ Model successfully loaded: {asl_model.model_type.upper()}")
        print(f"📱 Device: {asl_model.device}")
        print(f"🧠 Model parameters: {sum(p.numel() for p in asl_model.model.parameters()):,}")
